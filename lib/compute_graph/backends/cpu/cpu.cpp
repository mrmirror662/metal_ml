...
void Executor::visit(cg::MatMulNode& node) {
    ...
    for (int b = 0; b < batch; ++b) {
        tiled_matmul(A.data.data() + b * M * K,
                     B.data.data() + b * K * N,
                     C.data.data() + b * M * N,
                     M, K, N);
    }
...
void Executor::visit(cg::ScaleNode& node) {
    ...
    Tensor out(in.shape, in.data);
    for (auto& v : out.data) v *= node.scalar;
...
void Executor::visit(cg::MapNode& node) {
    ...
    switch (node.op) {
        ...
        case cg::MapOp::Step:
            for (auto& v : out.data) v = v > 0.0f ? 1.0f : 0.0f;
            break;
...
void Executor::visit(cg::ReduceNode& node) {
    ...
    foreach_index(in.shape, [&](int /*flat*/, const std::vector<int>& idx) {
        ...
        float val = in.at(idx);
        switch (node.op) {
            case cg::ReduceOp::Sum:  out.data[out_off] += val; break;
            case cg::ReduceOp::Max:
                if (idx[axis] == 0 || val > out.data[out_off]) out.data[out_off] = val; break;
            case cg::ReduceOp::Min:
                if (idx[axis] == 0 || val < out.data[out_off]) out.data[out_off] = val; break;
            case cg::ReduceOp::Mean: out.data[out_off] += val; break;
        }
    });

    if (node.op == cg::ReduceOp::Mean) {
        float n = (float)in.shape[axis];
        for (auto& v : out.data) v /= n;
    }
...
void Executor::visit(cg::BroadcastNode& node) {
    ...
    for (int o = 0; o < outer; ++o)
        for (int c = 0; c < node.count; ++c)
            std::memcpy(out.data.data() + (o * node.count + c) * inner,
                        in.data.data()  + o * inner,
                        inner * sizeof(float));
...
void Executor::visit(cg::ReshapeNode& node) {
    ...
    Tensor out(node.new_shape, in.data);   // same data, new shape
    results_[&node] = std::move(out);
}
    ...
    for (int n = 0; n < N; ++n) {
        for (int oh = 0; oh < Hout; ++oh) {
            for (int ow = 0; ow < Wout; ++ow) {
                ...
                float* dst = out.data.data() + row * patch_size;
                for (int c = 0; c < C; ++c) {
...
void Executor::visit(cg::Col2ImNode& node) {
    ...
    for (int n = 0; n < N; ++n) {
        for (int oh = 0; oh < Hout; ++oh) {
            for (int ow = 0; ow < Wout; ++ow) {
                int row = ((n * Hout) + oh) * Wout + ow;
                const float* src = In.data.data() + row * patch_size;
                for (int c = 0; c < C; ++c) {
                    for (int ky = 0; ky < kH; ++ky) {
                        int ih = oh * s + ky - p;
                        for (int kx = 0; kx < kW; ++kx) {
                            ...
                            int col = (c * kH + ky) * kW + kx;
                            out.data[((n * C + c) * H + ih) * W + iw] += src[col];
                        }
                    }
                }
            }
